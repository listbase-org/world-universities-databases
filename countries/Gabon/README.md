# Gabon — World Universities Database

1 records for **Gabon** (Gabon).

## Download

| Format | Link |
|---|---|
| **SQLite / SQL** (all countries) | [GitHub Releases](../../releases) |
| **Excel / CSV / PDF** (Gabon only) | [listbase.org](https://listbase.org/en/education/) |

## Preview

| name | country | alpha_two_code | state_province | domains | web_pages |
| --- | --- | --- | --- | --- | --- |
| Université Omar Bongo | Gabon | GA |  | uob.ga | http://www.uob.ga/ |

## Schema

```sql
SELECT * FROM universities WHERE country = 'Gabon';
```

## Stats

- **1** records
- Country: **Gabon** (Gabon)

## Browse & Download

📊 **[Download Gabon data as Excel/CSV/PDF](https://listbase.org/en/education/)**

---

Part of [World Universities Database](https://github.com/listbase-org/world-universities-databases) — 10,239 records across 202 countries.

Made with data from [ListBase.org](https://listbase.org) — Free Reference Tables & Lists

## All Entries (1)

- Université Omar Bongo
