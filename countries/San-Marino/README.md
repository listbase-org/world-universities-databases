# San Marino — World Universities Database

1 records for **San Marino** (San Marino).

## Download

| Format | Link |
|---|---|
| **SQLite / SQL** (all countries) | [GitHub Releases](../../releases) |
| **Excel / CSV / PDF** (San Marino only) | [listbase.org](https://listbase.org/en/education/) |

## Preview

| name | country | alpha_two_code | state_province | domains | web_pages |
| --- | --- | --- | --- | --- | --- |
| University of San Marino | San Marino | SM |  | unirsm.sm | http://www.unirsm.sm/ |

## Schema

```sql
SELECT * FROM universities WHERE country = 'San Marino';
```

## Stats

- **1** records
- Country: **San Marino** (San Marino)

## Browse & Download

📊 **[Download San Marino data as Excel/CSV/PDF](https://listbase.org/en/education/)**

---

Part of [World Universities Database](https://github.com/listbase-org/world-universities-databases) — 10,239 records across 202 countries.

Made with data from [ListBase.org](https://listbase.org) — Free Reference Tables & Lists

## All Entries (1)

- University of San Marino
