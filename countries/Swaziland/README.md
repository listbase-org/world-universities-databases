# Swaziland — World Universities Database

1 records for **Swaziland** (Swaziland).

## Download

| Format | Link |
|---|---|
| **SQLite / SQL** (all countries) | [GitHub Releases](../../releases) |
| **Excel / CSV / PDF** (Swaziland only) | [listbase.org](https://listbase.org/en/education/) |

## Preview

| name | country | alpha_two_code | state_province | domains | web_pages |
| --- | --- | --- | --- | --- | --- |
| University of Swaziland | Swaziland | SZ |  | uniswa.sz | http://www.uniswa.sz/ |

## Schema

```sql
SELECT * FROM universities WHERE country = 'Swaziland';
```

## Stats

- **1** records
- Country: **Swaziland** (Swaziland)

## Browse & Download

📊 **[Download Swaziland data as Excel/CSV/PDF](https://listbase.org/en/education/)**

---

Part of [World Universities Database](https://github.com/listbase-org/world-universities-databases) — 10,239 records across 202 countries.

Made with data from [ListBase.org](https://listbase.org) — Free Reference Tables & Lists

## All Entries (1)

- University of Swaziland
